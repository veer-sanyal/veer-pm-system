...
There isn't a single hardcoded number on the dashboard I shipped this month. Every figure is computed from the raw data at build time. Here's the small decision behind that rule.

When I built the chart for India's payments for use of intellectual property, the number in my head was "about 10x since 2005." Round, memorable, easy to drop into a caption. I almost did.

Computed from the source data, the real figure is 29.4x: $0.67B in 2005 to $19.8B in 2025.

The point isn't that I caught a wrong number. It's why I'd stopped typing numbers in by hand at all. A hardcoded constant is a guess that's wrong the day you write it and stale every day after. When the WTO updates the dataset, a "10x" sitting in a caption just rots there until someone notices. A figure computed from source is right now and updates itself on the next build. One rule, no maintenance, no stale captions to hunt down later.

That same discipline is why I trust the headline result: the India MSME service sector most exposed to digital-trade policy isn't software, the famous export line. It's consulting and professional services. Not a result I went looking for, just the one the pipeline returned.

The dashboard ranks the 11 service categories by exposure to policies like the WTO e-commerce moratorium, built for one user: a trade-policy analyst deciding which sectors are worth a deep dive. Every score links back to its source, and the one thing the public data genuinely can't support, a firm-level link between a sector's trade and its small-firm base, is labeled on the page as a modeled assumption rather than buried.
