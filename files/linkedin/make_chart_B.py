"""Reproducible generator for chart_B_ip_licensing_v3.png (LinkedIn post v6).
...
v6 reframe (2026-06-14): the story is build discipline, not catching a wrong
guess. A clean round "30x" would have been basically right (29.4x rounds to
~30x); the point is not hardcoding even a number you'd get right, because it
goes stale on the next data update. The dashed line marks where a round 30x
would sit -- just above the computed 2025 value, i.e. basically right. This
replaces v2's "~10x guess" line, which leaned on the weaker error-catch story.
...
start, end = ip[0], ip[-1]
mult = end/start
guess30 = start*30                                          # where a round "30x" would sit -- basically on the real value
...
ax.axhline(guess30, ls=(0,(6,4)), lw=1.6, color="#b9b2a3", zorder=1)
ax.text(2005.2, guess30+0.4, "a round “30x” would have been basically right", color="#8a8273",
...
fig.text(0.066,0.905,"Even a clean “30x” I’d get right by hand goes stale. Computed: 29.4x.",
...
out = os.path.join(os.path.dirname(os.path.abspath(__file__)), "chart_B_ip_licensing_v3.png")
