...
- **Fewer calendar-write prompts:** reconcile and Sunday sessions retitle blocks and write briefings weekly, and each write currently needs approval (the Jul 6 retitle denial was this). To pre-approve, add under `permissions.allow` in `.claude/settings.json`: `"mcp__claude_ai_Google_Calendar__create_event"` and `"mcp__claude_ai_Google_Calendar__update_event"` (deletes stay prompted). Veer adds this himself — the assistant is correctly blocked from widening its own permissions.
- **iMessage visibility (sensitive, OFF until Veer explicitly opts in):** the externship team channel lives in iMessage, which reconcile cannot see (a recurring blind spot in the logs). macOS stores it at `~/Library/Messages/chat.db`; with Full Disk Access granted to the terminal, reconcile could query it read-only for tracked names. This means the system reads personal messages — opt in deliberately, and only then add the scan to `/reconcile`.
