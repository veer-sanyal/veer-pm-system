...
- **iMessage visibility — Status: ON (opted in 2026-07-11; two false alarms from the blind spot that week forced the call).** The externship team channel lives in iMessage. The scan is wired in `/reconcile` and runs while this line says ON: read-only queries of `~/Library/Messages/chat.db` against the live tracked names + the externship team thread. Requires the terminal app to have Full Disk Access (System Settings → Privacy & Security). To opt out: change ON back to OFF.
