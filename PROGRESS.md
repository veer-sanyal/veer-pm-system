...
- **Decision (2026-06-27): reconnect FIRST, build later.** Text the dad directly this week (get the number from Cole): warm, accountable, low ask (learn about his work + help however useful — not "give me the internship back"). Only build a concrete artifact if a real need surfaces in the call. Ready draft text provided this session.
- Next move: get the dad's number from Cole; send the text before end of week; capture his name + company name once known and update this entry. Treat as relationship-preservation + an outreach rep, not internship-recovery (keeps the stakes low enough to actually send).
