...
- **Sleep-fix workstream executed (later same session).** Ran the built-in deep-research workflow (5 angles, 22 sources, 24/25 claims verified by 3-vote adversarial check). Headline: a rigidly fixed wake time held all 7 days + no naps is the core lever — it rebuilds homeostatic pressure so the late screens collapse on their own (Bootzin Stimulus Control Therapy; Verreault 2024 NMA of 23 RCTs). Morning light + 0.5mg afternoon melatonin are ~0.5h/day accelerants, not the engine (Crowley & Eastman 2015); AASM/Auger 2015 gives morning-light-alone NO recommendation for adults. Blue-light glasses and "no screens 2h before bed" are weak/myth — the lever is removing the phone. Adjudicated Veer's "initialize" wake-anchored idea: the hard fixed anchor is the active ingredient, so "initialize" is scoped to a transition-phase tool + graceful degradation, not a daily rebuild from drifting wake. **Asked Veer the 4 build-affecting questions; he chose:** stepped advance, both circadian levers, phone across the room (partial friction, so the if-then plans carry more weight), track the 3 indicators. Built `context/sleep-protocol.md` (owns the wake target + ladder) and `files/sleep-circadian-research-2026-06-25.md` (evidence base); wired "initialize" + the wind-down cue + wake-anchored delivery into SYSTEM-PROMPT.md and daily-briefing-instructions.md; reconciled the false fixed-9am hard-codes in about-me.md / scheduling-blueprint.md to point at the protocol. Step 1 (wake 11:15) starts Fri Jun 26; hard 9:00 lock ~Jul 5, held through August. Also this session: renamed daily-email-instructions.md -> daily-briefing-instructions.md (calendar delivery, not email; the old name implied an inbox).
- **Sleep config refined (same session, second pass on the levers) — supersedes the choices in the bullet above.** Final config: pure fixed 9:00 wake from Fri Jun 26 (no ramp, cold-turkey path), behavioral-first (morning light + 0.5mg melatonin held as a ~Jul 9 escalation, only if the schedule has not moved), phone stays near with app-limits + a wind-down cue (no relocation), track 2 indicators (wake time + the time he decides to sleep; onset latency dropped). sleep-protocol.md rewritten to match; "initialize" simplified to log-2-indicators + confirm-9:00 + graceful-degradation (no ladder). Also reconciled with master: deferred to the canonical state.json write-after-every-item fix (92bb632) and dropped my redundant duplicate; the feature branch is now master + rename + sleep integration.
