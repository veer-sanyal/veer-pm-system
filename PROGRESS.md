...
- **Study trace (P2 tutor session):** 2026-06-25: 3 items, 100% accuracy, topics: metric trees + NSM/guardrail. Metric-tree goal-vs-NSM crux **passed cold unaided**; guardrails taught then **2 unaided-correct retrievals → graduated past first-contact** (bloom Apply, mastery ~0.56). metric_tree mastery 0.45→0.62. Two tutor system changes this session: (1) **confidence number is now a hard gate** (no grade until Veer gives a 0-100, every item) and (2) **state.json now writes after every item, not just at close** — the direct fix for the Jun 23 lost-session bug. Both wired into `study-tutor.md` / `pedagogy.md` / `state-spec.md`.
