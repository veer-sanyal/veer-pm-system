...
- **Claude Corps Fellow application (NEW, decided 2026-06-14):** Veer is applying and is open to a **leave of absence from Purdue** for the 12-month fellowship (Oct 2026 – Oct 2027); deadline **Jul 17, 2026** (rolling; earlier = better). Resume + both 350-word essays + full form answer key are drafted in `applications/Claude Corps/`. **Progress: BOTH gating courses DONE** — Claude 101 (2026-06-15) and **AI Fluency: Framework & Foundations (2026-06-18)**, certificates saved to `applications/Claude Corps/Skilljar_Claude101_Certificate.pdf` and `Skilljar_AIFluency_Certificate.pdf`. The course gate is fully cleared. **All [CONFIRM] fields now resolved (2026-06-18):** work authorized (no sponsorship), 18+, email = personal, education = some college/no degree, start date 10/13/2026, heard via family (father), mentor box checked, CodePath = no. Certs merged into `Skilljar_Certificates_Combined.pdf` for the single-document upload. **Only step left: Veer pastes the answer key into the Greenhouse form (job-boards.greenhouse.io/claudecorps/jobs/4250200009), attaches resume + combined cert, and hits submit.** Optional/voluntary fields (pronouns, accommodations, research opt-in, demographics) are his discretion and don't affect selection. Stage → Ready. Note: this path *replaces* the Summer 2027 PM-internship plan rather than supplementing it — revisit the four-pillar plan deliberately if he advances.
