...
6. **`purdue-fall-registration` — LARGELY CLOSED 2026-08-21: Veer submitted.** Registered at 14 cr: SYS 35000 (TR 9:00) · STRT 35200IBE 30640 (TR 10:30) · **VIP 37920 Lab 11958-022 (R 3:30, AI for Education)** · SCOM 36100IBE (TR 4:30) · QM 30600 (MWF 4:30) · IBE 29150 async. **Mon/Wed/Fri are single-class days starting 4:30pm.** Residue, all for the **Alec meeting Mon Aug 24** (BoilerConnect says 2:30-3:00pm ET, Veer's own email says 11:30 — confirm which): (a) **UCC Science, Technology & Society is now uncovered** — EPCS 20100 was the only thing carrying it and it came off the schedule; Veer's call is that VIP replaces EPICS and he will confirm with Alec; (b) sequence QM 30500 into spring deliberately (deferred, gates nothing — STAT 35000 already satisfies every downstream prereq); (c) the audit offers only Financial Engineering and Student Specialty Area, so ask how Data Analytics gets declared; (d) classification still reads Sophomore at 69 credits; (e) MIS 38800 is fall-only per the catalog, confirm the spring IBE cohort section is real. Add deadline Thu Aug 28.
7. **`vip-aie-seat` — CLOSED 2026-08-21, same day it opened.** He took the last seat (`1 / 18` -> `0 / 18`).
8. **`civics-literacy` — OPENED 2026-08-21. A graduation requirement sitting at ZERO** and nobody has mentioned it: no approved course, no event participation, no podcast, no knowledge test. Applies to everyone entering summer 2021 or later. Usually cheap (podcast track + knowledge test). First physical action: open the Student Civics Literacy dashboard linked from the myPurduePlan audit and pick a pathway.
...
- **BOTH SUBMITTED: Amex GCS PM Intern (26011916) on Aug 21, IBM PM Intern 2027 (req 128509) on Aug 22.** That takes the cycle from 2 submissions to 4, and doubles it in two days. Amex was the first application filed against a hard deadline (Sep 30) rather than a rolling posting. Docs: `applications/IBM/`, `applications/American Express/`. **No action is owed on either; both are now waiting on the companies.** Next cheap shot is the second Amex PM req (Global Merchant & Network Services, NYC) already sitting as a Researching row.
- **Jane Street SP internship package BUILT 2026-08-21, Stage Ready, NOT submitted** (`applications/Jane Street/`). NYC, May-August, base $225,000, full-time start Sept 2028 matches May 2028 grad. No CS screen, finance background explicitly not expected. Sept 30 is encouraged, not hard. **Nothing owed until Veer decides to file it.**
...
- **Sun Aug 23 session must cover:** W1-of-classes plan with the actual class schedule written to the calendar (there are zero class events on it), the Pacific→Eastern shift of every recurring block, the sleep walk-back verdict, the P2 re-point verdict (still zero verified reps, state.json cold since Jul 3), and accept/kill on the remaining Researching pipeline rows (2 moved to Ready Aug 21; a second Amex PM req was added).
