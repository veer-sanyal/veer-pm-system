...
- **Sleep-fix workstream LIVE (2026-06-25) — upstream of all four pillars.** The calendar spine was anchored to a fixed 9am wake Veer kept missing (waking 11am-1pm, screen-driven, behavioral not insomnia); fixing it unblocks the outreach + study that kept slipping because the day started at noon. Protocol owner: `context/sleep-protocol.md` (evidence base: `files/sleep-circadian-research-2026-06-25.md`). Config chosen: hard fixed 9:00 wake all 7 days from Fri Jun 26 (no ramp), behavioral-first (no melatonin/light yet — held as escalation if not moving by ~Jul 9), phone stays near with app-limits + wind-down cue, track 2 indicators (wake time + decided-bedtime). Held through August to lock before classes ~Aug 24. Each morning Veer sends "initialize"; daily indicators logged in PROGRESS "System signals."
...
- **Sleep schedule (highest-leverage, active):** run `context/sleep-protocol.md` — hard fixed 9:00 wake from Fri Jun 26 (no ramp), behavioral-first; escalation check ~Jul 9 (add morning light then 0.5mg melatonin only if not moving). Daily "initialize" each morning. Upstream of Pillars 2 and 3 (study + outreach kept slipping because the day started at noon).
