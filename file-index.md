...
| `.claude/commands/` | Slash-command protocols, loaded only when invoked: `reconcile` (daily reality-sync), `initialize` (morning wake signal), `sunday` (weekly session + system maintenance + monthly health check), `apply` (resume/cover letter), `tutor` (study mode), `debrief` (post-call loop close, added 2026-07-08). |
| `.claude/hooks/session-start.sh` | SessionStart hook (registered in `.claude/settings.json`): prints the per-session status banner — stamp staleness, live tripwire headers, uncommitted/unpushed git state, telemetry-gap detection. Added 2026-07-08. |
| `session-log.jsonl` | Session telemetry: one JSON line per session (schema in the file's first line; v2 2026-07-08 adds `tripwires_opened`/`tripwires_closed` slugs so tripwire latency is computable). Feeds the monthly system health check in `/sunday`. Rolls to `session-log-<year>.jsonl` each January. |
...
| `PROGRESS.md` | OWNS the running narrative: the day-by-day dated log across all four pillars. Read the tail (~3-4 weeks) by default; older months roll into `PROGRESS-archive.md`. PRIVATE (real names + strategy): keep this repo private. |
| `ledgers.md` | OWNS the standing ledgers, split out of PROGRESS.md 2026-07-08: Pillar 2/3/4 running ledgers (regenerated each Sunday), the external-relationships log (Dr. G, Trice, alumni, Miller, TikTok), and system signals. PRIVATE. |
...
| `networking/` (advising) | `alec-gramm-advising-prep-2026-08-24.md` — prep sheet for the Mon 2026-08-24 academic-advising meeting (Alec Gramm). Carries the registered Fall 2026 schedule, the five ranked questions (STS coverage after EPCS 20100 came off, how to declare Data Analytics, sequencing QM 30500 into spring, the stale Sophomore classification, MIS 38800 fall-only), and the secondary items (Civics Literacy at zero, IBE 29150/29130, VIP needing 4 credits). Written 2026-08-21.
| `networking/` | Per-contact call-prep sheets, one file per scheduled call, grounded in the networking research docs: `Anicito-Fidelity-call-prep.md`, `LaCroix-Elanco-call-prep.md`, `dr-g-call-prep-2026-06-18.md` (historical), `naveen-trice-call-prep-2026-07-29.md` (conversion call, not informational — includes live company research), `rcac-ahlawat-call-prep-2026-08-06.md` (RCAC GenAI Studio meeting, feeds STICK S-01/OD-163). PRIVATE (real names). Each new call gets a sheet here, cross-linked to its `alumni-tracker.md` row. |
| `networking/` (externship work products) | Client-facing and internal deliverables for the Scouting America externship, added 2026-08-04. **CLIENT-prefixed files are the sendable versions**; the unprefixed audits are internal and carry process notes. `CLIENT-sagamore-website-audit.md` · `CLIENT-sagamore-social-audit.md` · `sagamore-website-audit-2026-08-04.md` (internal) · `sagamore-social-audit-2026-08-04.md` (internal) · `sagamore-posting-toolkit-2026-08-04.md` (the tooling decision and why MCP/API routes were rejected) · `canva-process-2026-08-04.md` (step-by-step Bulk Create process) · `sends-2026-08-03.md` (the five owed messages). Generated by `projects/sagamore/tools/make-client-copy.py` and `projects/sagamore/tools/make-bulk-create-csv.py`. **`audit-verification-pass-2026-08-07.md` (added 2026-08-07) is the authority on which audit claims are true** — a signed-in real-browser re-run of every checkable finding. It killed four website findings and the social audit's central thesis; the two CLIENT files are corrected, the two internal files carry a superseded banner and are deliberately left unrewritten. Read it before quoting either audit. `externship-final-meeting-prep-2026-08-07.md` is the Fri Aug 7 team-meeting sheet. |
...
| Knowledge freshness | `knowledge-freshness.md` | OWNS the staleness tiering of the `context/` research docs: LIVING (verified-as-of date + event trigger, swept each Sunday) vs STATIC (durable). Added 2026-07-24 (audit F1). The `/sunday` freshness pass reads it; routing points here before trusting a research doc for a time-sensitive fact. |
...
| Sleep protocol | `sleep-protocol.md` | OWNS the wake-time target + live sleep-fix config: hard fixed 9:00 wake (no ramp), behavioral-first with a ~Jul 9 light/melatonin escalation, phone-near + app-limits wind-down, the 2 tracked indicators, bad-night/weekend rules, and the "initialize" behavior. Added 2026-06-25. Evidence base in `files/sleep-circadian-research-2026-06-25.md`. |
