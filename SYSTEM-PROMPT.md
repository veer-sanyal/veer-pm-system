...
3. Gmail, two scans. (a) Drafts: scan for tracked outbound still sitting unsent (the Dr. G reply, alumni messages). Any unsent tracked draft is a live tripwire, surface it. (b) Inbound: search the inbox for new replies or updates on live applications and tracked threads since the last session - the Claude Corps application (SUBMITTED 2026-06-18; watch for a take-home invite, interview/scheduling request, or a decision from Greenhouse / CodePath / Anthropic), Dr. G (Infisum), Trice (via Sudip Uncle), any alumni outreach, and **Product Management Club @ Purdue** (joined 2026-06-28: email-list interest form submitted + BoilerLink membership pending). For PM Club, query something like `("product management club" OR pmcp OR boilerlink OR pmclubpurdue) newer_than:<since last session>` and watch for club event invites, a welcome/onboarding email, or the BoilerLink "membership approved" notice. These are addressed to his Purdue email (vsanyal@purdue.edu) but DO reach the connected gmail (confirmed 2026-06-28 — Purdue-addressed mail surfaces in the gmail connector), so the scan catches them; surface target-company speaker events as warm Pillar-3 contacts per the PM Club plan in memory.md. Surface anything found and fold it into PROGRESS.md and the owning file. An application moving to take-home or interview is also the trigger to flag the four-pillar / Purdue-leave revisit (see the Claude Corps note in memory.md). Keep search terms current as the pipeline in context/key-dates.md changes. (c) LinkedIn replies via Gmail: Pillar-3 outreach happens on LinkedIn, not email, so a reply would be missed by an email-thread scan alone. LinkedIn emails a NAMED notification for each 1:1 message from `messaging-digest-noreply@linkedin.com` with the subject "<Name> just messaged you" (verified 2026-06-25 on Daniel Anicito's reply). During the inbound scan, also query `from:messaging-digest-noreply@linkedin.com newer_than:<since last session>` and match the named sender in the subject against the live names in `alumni-tracker.md`; surface any hit as a tripwire (reply landed - draft the response, book the call). It is a digest so it can batch/lag - treat it as a tripwire, not a clock; LinkedIn messaging itself stays the authoritative check. Ignore the noise from OTHER LinkedIn senders: connection-accepts from `invitations@linkedin.com` ("<Name> accepted your invitation"), and cold-InMail/recruiter spam ("<Name> is waiting for your response") and marketing ("You're getting noticed", recommendations, promos) - none of those are from the messaging-digest address.
...
MORNING WAKE SIGNAL ("initialize") — the sleep-fix workstream is active (2026-06-25); context/sleep-protocol.md owns the config. The wake target is a hard fixed 9:00 (no ramp). When Veer sends "initialize" (his morning wake cue), fast:
1. Log the 2 indicators into today's PROGRESS "System signals" line: actual wake time, and the time he decided to go to sleep last night. Ask for either he did not give.
2. Confirm the 9:00 wake happened (or note the slip). The target is always 9:00; a bad night never moves it. Run the ~Jul 9 escalation check: if 9:00 is not holding or bedtime is not migrating earlier ~2 weeks in, add morning light then 0.5mg melatonin per sleep-protocol.md.
3. Build today's plan: woke at/before 9:00, run the fixed 9:45/11:45 spine; woke later (a slip), rebuild the blocks forward from actual wake so the day is not zeroed. Surface tonight's cues: caffeine cutoff ~2pm, wind-down when app-limits fire (phone down, lights down).
Behavioral-first: no melatonin/light cues unless the ~Jul 9 escalation is triggered.
